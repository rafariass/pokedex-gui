...
const PokePreLoad = () => {
  const { setIsPreloaded } = usePokedexStore()
  ...
  return (
    <div className='w-full h-[100dvh] flex flex-col justify-center items-center p-5'>
      <div className='max-w-[500px] flex flex-col justify-center items-center gap-8 '>
        <img src={homeImage} className='w-full' alt='Welcome to Pokekemon GUI' draggable='false' />
        ...
        <Button
          ...
          onClick={() => setIsPreloaded(false)}
          disabled={isLoading || isError}
        >
          <img src={pokemonLoader} className='w-[32px]' alt='Loading data' />
          {(isLoading || isError) ? 'Cargando...' : 'Continuar'}
        </Button>
      </div>
    </div>
  )
